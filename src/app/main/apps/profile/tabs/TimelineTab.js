...
const dataa = [{name:"tenant"},{name:"tenant"},{name:"tenant"}];

function displayLicense(data) {
  const arr = data?.map((ele) => {
    return <Chip label={ele?.name} sx={{marginRight:"5px"}} variant="outlined" />
  })
  return arr
}
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="w-full">
      <div >
        <div className="flex flex-col w-full ml-32 ">
          <Card component={motion.div} variants={item} className="flex  w-full px-32 pt-24 mb-20">
            <div className="flex  pb-16">
              ...
              <div className='flex  w-full m-4'><div className='w-1/4 font-bold'>First Name</div> <div className='w-1/2'>{user?.data?.firstName}</div></div>
              <div className='flex  w-full m-4'><div className='w-1/4 font-bold'>Last Name</div> <div className='w-1/2'>{user?.data?.lastName}</div></div>
              <div className='flex  w-full m-4'><div className='w-1/4 font-bold'>Email</div> <div className='w-1/2'>{user?.data?.email}</div></div> 
              <div className='flex  w-full m-4 mt-4'>
                  <div className='w-1/4 mt-4 font-bold'>Tenant</div>
                    {user?.tenant.lenght == 0 ?<div>No tenant available</div>:<div className='w-1/2'><div className='flex  w-full'><div className='flex  w-1/2 my-4 font-medium'>Name</div><div className='flex  w-1/2 my-4 font-medium'>Licenses</div></div>{user?.tenant?.map((ele)=><div className='flex  w-full mt-4'><div className='flex  w-1/2 pt-4'>{ele?.name}</div><div className='flex  w-1/2'>{displayLicense(ele.license)}</div></div>)}</div>}</div> 
              </div>
              </Card>
              </div>
      </div>
