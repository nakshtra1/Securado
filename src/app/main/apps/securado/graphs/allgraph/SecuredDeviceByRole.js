...
function SecuredDeviceByRole() {
  const [loading, setLoading] = useState(true);
  const [data,setData] =useState([]);
  // const data =[
  // {category:"Laptop" ,value:32},
  // {category:"Desktop",value:31},
  // {category:"Server",value:9},
  ...
  const getData=async()=>{
    const response = await axios.get("http://localhost:3600/api/graph/securedDevicesbyRole");
    if(response?.data?.success)
    {
      // console.log(response.data,"response in role")
       convertObjectToArray(response.data.securedDevicesbyRole)
    }
    else{
...
export default memo(SecuredDeviceByRole);