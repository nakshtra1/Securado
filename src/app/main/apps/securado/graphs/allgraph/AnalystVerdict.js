...
const AnalystVerdict = () => {
  ...
  const getData=async()=>{
    const response = await axios.get("http://localhost:3600/api/graph/analystVerdictThreats");
    if(response?.data)
    {
    console.log(response.data,"response in verdict t t t t tOS")
       convertObjectToArray(response.data.analystVerdictThreats)
    }
    else{
  ...
  return (
    <Paper className="flex flex-col flex-auto shadow rounded-2xl overflow-hidden p-24">
      ...
      <StackedChart data={data} title="Undefined"/>
      </div>
      <div className="mt-32">
