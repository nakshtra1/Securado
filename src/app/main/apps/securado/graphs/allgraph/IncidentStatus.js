...
import HorizonatlStackedBar from "../HorizonatlStackedBar"
const IncidentStatus = () => {
  ...
  //   throw new Error("Labels and Series arrays must have the same length.");
  // }
  ...
  const getData=async()=>{
    const response = await axios.get("http://localhost:3600/api/graph/incidentStatusThreatinfo");
    if(response?.data?.success)
    {
    console.log(response.data,"response in securedDevices OS")
       convertObjectToArray(response.data.incidentStatusThreatinfo)
    }
    else{
...
<HorizonatlStackedBar data={data} title="Unresolved" />
    </div>
    <div className="mt-32">
